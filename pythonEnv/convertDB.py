...
old_db_conn = sqlite3.connect('ingresosVieja.db')
old_cursor = old_db_conn.cursor()

# Conexión a la nueva base de datos
new_db_conn = sqlite3.connect('ingresos.db')
...
for factura in facturas:
    ...
    numero, receptor, emisor, fecha, unidades, concepto, importeTotal, irpf, detalles, formaDePago = factura

    # Insertar los datos en la nueva tabla facturas
    new_cursor.execute('INSERT INTO facturas (numero, receptor, emisor, fechaEmision, fechaVencimiento, unidades, concepto, importeTotal, irpf, detalles, formaDePago) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                       (numero, receptor, emisor, fecha, 'PENDIENTE', unidades, concepto, importeTotal, irpf, detalles, formaDePago))
